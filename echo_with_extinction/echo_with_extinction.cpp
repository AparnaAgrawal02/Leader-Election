...
int main(int argc,char *argv[])
{
    ...
    int total_sent = 0;
    int total_recv = 0;
    MPI_Reduce(&num_sent,&total_sent,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
    MPI_Reduce(&num_recv,&total_recv,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
    if (rank == simulator){
        cout << "Total number of messages sent: " << total_sent << endl;
        cout << "Total number of messages received: " << total_recv << endl;
    }
    else {
        cout << "Leader elected by process " << rank << " is " << winner << endl;
    }
    MPI_Finalize();
    return 0;
